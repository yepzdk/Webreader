...
public struct HiddenPhrases: Equatable {
    ...
    function readerHideBlocks(root, phrases) {
      ...
      // One list, shared with the picker: the two passes must agree on what counts as a
      // thing rather than as part of a sentence.
      var BLOCKS = '\(BlockPicker.blocks)';
      var MEDIA = '\(BlockPicker.media)';
    ...
    /// Where the floating "Hide text" button used to be. It was driven by the document
    /// selection, which is why it is gone: it repositioned on every `selectionchange` during
    /// a drag, and on touch it could only try to dodge the system's own Copy / Look Up
    /// callout. `BlockPicker` replaces the gesture — a mode you turn on, in which the
    /// article is aimed at rather than selected — and the phrase list it fed is unchanged,
    /// including the `readerHide` message and everything above this line.
}
