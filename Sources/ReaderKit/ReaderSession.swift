...
/// the three drifting.
///
/// Not thread-safe and not meant to be: every host drives it from its UI thread, and the
/// one thing that genuinely runs elsewhere — fetching feeds — is an `async` method that
/// touches no session state until it returns.
public final class ReaderSession {
    ...
    public func navigationFinished(url: URL?, generator: String) -> [ReaderCommand] {
        ...
        case .startPage:
            ...
            return [pushSettings(), .fetchSuggestions]
        case .settings:
            pageState.restored(generator: generator)
            return [pushSettings()]
        default:
            // Including the reader, which the extraction script's own sentinel recognises.
