...
**Removing things from an article.** The framed-minus button in the chrome turns the article into something you aim at: hover (or Tab to) a block and it is outlined, click it and it goes — text, pictures and the space it held. **Undo** is in the bar the mode puts at the foot of the window, and **Done** or **Esc** leaves. What you remove is kept in *this article's* saved copy, so a recents row and an offline open show it as you left it; **⌘R** fetches the publisher's version again. Nothing listens to your selection: selecting text in the reader is the browser's own business again.

**Hidden text.** The other half, for boilerplate that recurs: lines that survive extraction — "Artiklen fortsætter efter annoncen", "Advertisement" and the like — are removed from *every* article. A paragraph is dropped only when its entire text is one of the phrases, never when it merely contains one. Teach it new ones from the remove-blocks bar: take out a short label and press **Hide everywhere**. The eye-off button next to **Aa** shows how many blocks the current article lost and lists the phrases — the ones that hit this article first, with their count — each with a remove control.
