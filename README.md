...
**Getting back.** Every page except the start page has a Home button in the top-left corner (**⇧⌘H** / **Ctrl+Shift+H** does the same), including the offline page — so a failed load is never a dead end. On the start page that corner holds **Settings** instead.
...
**Article images.** Recent articles on the start page carry a small thumbnail when the article named a lead image of its own (`og:image`). Turn them off with **Aa → No images**: with the setting off the page asks the publishers for nothing at all, and images already stored stay stored. Thumbnails are requested without a referrer, and only over `http`/`https`.
